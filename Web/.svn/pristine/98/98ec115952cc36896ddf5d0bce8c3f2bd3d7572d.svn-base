 ...
 */
	...
	$.mcDropDownMenu = function(el, list, options){
		...
		$divInput = $('<div class="mcdropdown"><a href="#" tabindex="-1"></a><input type="hidden" name="' + (el.name || el.id) + '" id="' + (el.id || el.name) + '" /></div>')
			.appendTo($('<div style="position: relative;display:inline-block; _zoom:1;*display:inline;align:top;"></div>'))
			.parent();
...
*/						
		...
		function showBranch(){
			...
			$ul.css({top: li_coords.top, left: li_coords.width - 30/*, zIndex: settings.baseZIndex + ((depth)*2)*/}).show();
