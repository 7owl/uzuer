...
*/
	public static void pushToCilent(String cid, String content,
			String platform, String devicetoken,String packageName,String type) throws Exception {
		final Map<String, String> map = new HashMap<String, String>();
		map.put("cid", cid);
		map.put("content", content);
		map.put("platform", platform);
		...
		new Thread(new Runnable() {
			public void run() {
				...
				if ("1".equals(platform)) {
					//安卓平台不分包直接推送
					ret = AndroidPush(cid,content,platform,type);
				}else if ("2".equals(platform)) {
					//IOS平台
					if ("com.supin.uzuer".equals(packageName)) {
						//appstore
						ret = IOSAppPush(content, devicetoken, cid,type);
						
					}else if ("com.supin.e.uzuer".equals(packageName)) {
						// 企业
						ret = IOSEPush(content, devicetoken,cid, type);
					}else {
						//不确定
						 IOSAppPush(content, devicetoken, cid,type);
						 IOSEPush(content, devicetoken,cid,type);
					}
				}else{
					 AndroidPush(cid,content,platform,type);
					 IOSAppPush(content, devicetoken, cid,type);
					 IOSEPush(content, devicetoken,cid,type);
				}
	...
	public static TransmissionTemplate getTemplate1(String content, String type) {
		
		Map<String, String> body = new HashMap<String,String>();
		
		body.put("content", content);
		//定义11位邮箱 催款 是20 ，实名认证是 12
		body.put("type", type);
		Gson gson = new Gson();
	...
	//Ios gettui 	企业账号
	private static IPushResult IOSEPush(String content ,String devicetoken, String cid, String type) throws RequestException
	{
		...
		if (result.getResponse().get("result").toString().equals("Offline")) {
			SingleMessage sm = new SingleMessage();
			sm.setOffline(true);
			sm.setData(getTemplate1(content,type));
...
//			System.out.println(ret0.getResponse());
	...
	// Ios getui   Appstore 
	private static IPushResult IOSAppPush(String content ,String devicetoken, String cid, String type) throws RequestException
	{
		IGtPush push = new IGtPush(host, appkey, master);
		Target target = new Target();
		target.setAppId(appId);
	...
	// Android 推送 
	private static IPushResult AndroidPush(String cid, String content, String platform, String type) throws RequestException
	{
		IGtPush push = new IGtPush(host, appkey, master);
		NotificationTemplate template = notificationTemplateDemo(content,type);
		SingleMessage message = new SingleMessage();
		message.setOffline(true);
		message.setOfflineExpireTime(24 * 3600 * 1000);
		message.setData(template);
...
//		alertMsg.setTitle("");
	...
	// 打开应用
	public static NotificationTemplate notificationTemplateDemo(String content, String type) {
		...
		Map<String, String> body = new HashMap<String,String>();
		body.put("content", content);
		//定义11位邮箱 催款 是20 ，实名认证是 12 锁 13
		body.put("type", type);
		Gson gson = new Gson();
		String mainbody = gson.toJson(body);
		template.setText(mainbody);
		// 配置通知栏图标
