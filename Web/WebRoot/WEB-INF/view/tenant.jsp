...
<html>
	...
	<body>
					...
					<tr>
						<td>
							...
							<a href="javascript:void(0)" onclick="openWindow()"  
    							class="easyui-linkbutton 02" iconCls="icon-add" plain="true" style="display:none;color:green;">添加</a>
	    ...
	    <!-- 身份认证 -->
	    <div id="w_idAuth" class="easyui-window" title="" iconCls="icon-save"
	    	 style="width:1000px;height:500px;padding:5px;">
	    	<div class="easyui-layout" id="layout" style="width:975px;height:450px;">
	    		<div region="west" border="false" split="true" style="padding:10px;background:#fff;border:1px solid #ccc;height:100%;width:230px;">
		    		<form id="idAuthForm">
		...
		<script type="text/x-handlebars-template" id="tmp_idInfo">
			<span style="color:red;">注意:红色表示用户实际信息;绿色表示用户上传的信息</span><br><br>
			<span style="color:red;display:inline-block;width=100px;">用户名:</span>{{username}}<br><br>
			
			<span style="color:red;display:inline-block;width=100px;">姓名:</span>{{first_name}}{{last_name}}<br>
			<span style="color:green;display:inline-block;width=100px;">姓名:</span>{{full_name_valid}}<br><br>

			<span style="color:red;display:inline-block;width=100px;">身份证号码:</span>{{id_card}}<br>
			<span style="color:green;display:inline-block;width=100px;">身份证号码:</span>{{id_card_valid}}<br><br>
			
			<span style="color:red;display:inline-block;width=100px;">身份认证:</span><select onchange="idAuthChange(this, {{id}})">
					<option value="0" {{isSelected identity_valid 0}} >不通过</option>
					<option value="1" {{isSelected identity_valid 1}} >通过</option>
				</select>
		</script>
		...
		<script type="text/javascript" src="<%=basePath%>js/system/tenant.js"></script>
	</body>
</html>
